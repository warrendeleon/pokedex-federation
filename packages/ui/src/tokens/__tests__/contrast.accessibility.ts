...
function expectPair(foreground: string, background: string, min: number, label: string): void {
...
describe('WCAG 1.4.3 - Contrast (Minimum)', () => {
  describe('text on the white card surface', () => {
    it('black name text', () =>
      expectPair(colours.black, colours.white, AA_NORMAL, 'black on white'));
  ...
  describe('text on the off-white screen surface', () => {
    it('black text', () =>
      expectPair(colours.black, colours.offWhite, AA_NORMAL, 'black on offWhite'));
    it('darkGrey text', () =>
...
describe('WCAG 1.4.11 - Non-text Contrast', () => {
  ...
  it.failing('stat-bar fill on the track (known: most type fills < 3:1; value shown as text)', () =>
    expectPair(
      typeColours.electric,
      colours.lightGrey,
      AA_LARGE,
      'electric fill on lightGrey track'
    )
  );
});
