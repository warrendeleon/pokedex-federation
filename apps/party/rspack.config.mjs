...
// --- partyApp: federated remote micro-app. A JS-only remote with no native project; it loads
// into the host's runtime. Exposes ./PartyStack (the party tab: the team of up to six Pokémon,
// read from the host-owned party slice). Shared deps are lazy (eager:false) so the remote
...
export default Repack.defineRspackConfig(env => {
  ...
  return {
    ...
    output: {
      ...
      path: `[context]/cdn/[platform]/partyApp/${REMOTE_VERSION}`,
      uniqueName: 'PokedexPartyApp',
    },
    module: {
      rules: [
        {
        ...
        extraChunks: [
          {
            include: /.*/,
            type: 'remote',
            outputPath: `cdn/${platform}/partyApp/${REMOTE_VERSION}`,
          },
        ],
      }),
